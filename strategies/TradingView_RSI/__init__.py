...
class TradingView_RSI(Strategy):
    ...
    def should_long(self):
        ...
        if (
            utils.crossed(self.rsi, 35, direction="above")
            and qty > 0
            and self.available_margin > (qty * self.price)
        ):
            return True
    ...
    def go_long(self):
        ...
        self.take_profit = qty, (
            self.price * self.hp["take_profit"]
        )  # Take profits at 10%
    ...
    def update_position(self):
        if utils.crossed(
            self.rsi, self.hp["xparam"], direction="below"
        ) or utils.crossed(self.rsi, 10, direction="below"):
            self.liquidate()
