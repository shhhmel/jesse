...
class TurtleRules(Strategy):
    ...
    def unit_qty(self, unit_risk_percent, dollars_per_point=1):
        ...
        return ((unit_risk_percent / 100) * self.balance) / (
    ...
    def update_position(self):
        ...
        if self.current_pyramiding_levels < self.vars["maximum_pyramiding_levels"]:
            if self.is_long and self.price > self.last_opened_price + (
                self.vars["pyramiding_threshold"] * self.atr
            ):
                qty = self.unit_qty(self.vars["unit_risk_percent"])
                self.buy = qty, self.price
                # self.log(f"atr={self.atr}, last price={self.last_opened_price}, cur price={self.price}, action: increase long position {qty}")

            if self.is_short and self.price < self.last_opened_price - (
                self.vars["pyramiding_threshold"] * self.atr
            ):
                qty = self.unit_qty(self.vars["unit_risk_percent"])
                self.sell = qty, self.price
        ...
        if (
            self.is_long
            and (
                self.entry_signal() == "entry_short"
                or self.exit_signal() == "exit_long"
            )
            or self.is_short
            and (
                self.entry_signal() == "entry_long"
                or self.exit_signal() == "exit_short"
            )
        ):
            self.liquidate()
            self.current_pyramiding_levels = 0
    ...
    def on_increased_position(self, order):
        ...
        if self.is_long:
            self.stop_loss = (
                abs(self.position.qty),
                self.price - self.vars["atr_multiplier"] * self.atr,
            )
