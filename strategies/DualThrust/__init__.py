...
class DualThrust(Strategy):
    ...
    @property
    def up_thurst(self):
        return self.anchor_candles[:, 1][-1] + self.hp["up_coeff"] * max(
            self.up_max_close - self.up_min_low, self.up_max_high - self.up_min_close
        )

    @property
    def down_thrust(self):
        return self.anchor_candles[:, 1][-1] - self.hp["down_coeff"] * max(
            self.down_max_close - self.down_min_low,
            self.down_max_high - self.down_min_close,
        )
