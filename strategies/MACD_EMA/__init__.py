...
class MACD_EMA(Strategy):
    ...
    def macd(
        self,
    ):  # this returns: macd, signal and hist which can be referenced as self.macd[0], self.macd[1] and self.macd[2], respectively
        return ta.macd(
            self.candles,
            self.hp["fastperiod"],
            self.hp["slowperiod"],
            self.hp["signalperiod"],
        )

    @property
    def ema(
        self,
    ):  # this returns a single value which is the 100EMA at the latest candle
